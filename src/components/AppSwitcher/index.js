...
export default class AppSwitcher extends Component {
  constructor(props) {
    ...
    this.facebookUrl =
      "https://www.facebook.com/plugins/page.php?href=https%3A%2F%2Fwww.facebook.com%2Fwilliam%2F&tabs=timeline&width=360&height=560&small_header=false&adapt_container_width=true&hide_cover=false&show_facepile=false&appId=263980326955302";
    this.iamPlusUrl = "https://iamplus.com";
    this.iamAngelUrl = "http://iamangelfoundation.org/";
  }

  componentDidMount() {
    fetchImages().then(mediaUrls => this.setState({ media: mediaUrls }));

    fetchMessages().then(conversations => this.setState({ conversations }));

    fetchTracksFromPlaylist().then(tracks => this.setState({ tracks }));
    ...
    fetchEvents().then(events => this.setState({ events }));
  }
  ...
  render() {
    const renderApp = appName => {
      switch (appName) {
        ...
        case "photos":
          return <Photos key={3} media={this.state.media} />;
          break;

        case "messages":
          return <Messages key={4} conversations={this.state.conversations} />;
          break;
        ...
        case "music":
          return <Music key={6} tracks={this.state.tracks} />;
          break;
        ...
        case "terms":
          return <TermsAndConditions key={11} />;
          break;

        case "privacy":
          return <PrivacyPolicy key={12} />;
          break;
