...
export default class Messages extends Component {
  ...
  render() {
    ...
    const defaultContent = <h4 className="unavailable">Sorry, messages currently unavailable.</h4>;
    const listMessages = conversations.length === 0 ? defaultContent : (
      conversations.map((conversation, index) => {

	if(conversation.messages.length === 0) return null;

	let summary = conversation.messages[conversation.messages.length - 1];
	return (
    ...
    const renderSelectedConversations = this.state.areConversationsDisplayed && (
